	...
	return (
		...
		>
			<div className="max-w-[1280px] mx-auto px-30 text-left">
				<p
					className={`text-[#6DC0C8] uppercase tracking-widest text-[15px] mb-1 ${oswald.className}`}
				>
					What We Do
				</p>
				<h2
					className={`text-[42px] font-light italic text-black mb-10 ${arapey.className}`}
				>
					Makeup Services
				</h2>
				...
				>
					{services.map((service, index) => (
						<SwiperSlide key={index}>
							<div className="w-full h-[420px] bg-[#FFF] overflow-hidden shadow-xl flex flex-col transition-all duration-300 transform hover:-translate-y-2 hover:bg-[#E0F4F6] group mb-8">
								{/* Image - 60% */}
								...
								<div className="flex-1 p-4 text-center flex flex-col justify-center transition-colors duration-300">
									<h3
										className={`text-lg text-black font-normal tracking-[.5px] mb-3 group-hover:text-black transition-colors duration-300 ${oswald.className}`}
									>
